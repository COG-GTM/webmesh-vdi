...
func (d *desktopAPI) buildRouter() error {
	...
	// Grafana proxy - requires a valid user session. A finer-grained permission
	// model for the individual dashboards doesn't fit well into the existing
	// paradigms yet.
	...
	r.PathPrefix("/api/grafana").Handler(d.ValidateUserSession(grafanaProxy))
